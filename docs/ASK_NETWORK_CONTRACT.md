...
relationship-substrate ask-network \
  ...
  --semantic-query "medical communications consulting" \
...
Semantic retrieval is optional. When `--semantic-query` is supplied, code may embed that query and rank candidates by existing person embeddings, but it must expose embedding coverage/readiness instead of translating the query into hidden categories or deterministic fit scores.
...
```json
{
  ...
  "constraints": {
    ...
    "sort": "relationship | semantic | null"
  },
  "limits": {
...
- `organization_context`
- `model_inputs`
- optional semantic query metadata and per-candidate embedding evidence
...
- If semantic retrieval is requested, candidates without person embeddings expose `missing:person_embedding`.
